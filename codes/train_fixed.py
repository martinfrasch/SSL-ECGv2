"""
train_fixed.py - FIXED training script for SSL-ECG with proper subject-wise CV

This script implements self-supervised learning for ECG stress detection using
CORRECT subject-wise k-fold cross-validation (no data leakage).

Key improvements over train.py:
1. Subject-wise cross-validation (no subject overlap between train/test)
2. Configurable random seeds for reproducibility
3. Feature normalization before downstream tasks
4. Validation checks to ensure no data leakage
5. Better logging and progress tracking

Usage:
    # Run with default settings (correct subject-wise CV)
    python train_fixed.py

    # Run with custom seed
    python train_fixed.py --random_seed 123

    # Compare with old method (for ablation study only - causes leakage!)
...
parser = argparse.ArgumentParser(description='SSL-ECG Training with Subject-Wise Cross-Validation')
parser.add_argument('--random_seed', type=int, default=42,
                    help='Random seed for reproducibility (default: 42)')
parser.add_argument('--data_split_seed', type=int, default=None,
                    help='Separate seed for data splitting (default: same as random_seed)')
parser.add_argument('--subject_wise', type=lambda x: (str(x).lower() == 'true'), default=True,
                    help='Use subject-wise CV (True) or within-subject CV (False - INCORRECT) (default: True)')
parser.add_argument('--normalize_features', type=lambda x: (str(x).lower() == 'true'), default=True,
                    help='Normalize features before downstream tasks (default: True)')
parser.add_argument('--validate_split', type=lambda x: (str(x).lower() == 'true'), default=True,
                    help='Validate no subject overlap in train/test (default: True)')
parser.add_argument('--data_tag', type=str, default='mecg',
                    help='Type of ECG data: mecg or aecg (default: mecg)')
...
parser.add_argument('--extract_data', type=int, default=0,
                    help='Extract data from raw files (1) or use existing (0) (default: 0)')
...
print(f"\n{'='*70}")
print(f"SSL-ECG Training Configuration (FIXED VERSION)")
print(f"{'='*70}")
print(f"Random seed: {args.random_seed}")
print(f"Data split seed: {args.data_split_seed}")
print(f"Subject-wise CV: {args.subject_wise} {'✓ CORRECT' if args.subject_wise else '✗ INCORRECT - CAUSES DATA LEAKAGE!'}")
...
print(f"Total folds: {args.total_folds}")
print(f"{'='*70}\n")
...
if not args.subject_wise:
    ...
    print("⚠️  For valid experiments, always use subject_wise=True.\n")
...
total_fold = args.total_folds  # Use configurable fold count
## hyper parameters
batchsize = 128
actual_batch_size =  batchsize * single_batch_size
log_step = 100
epoch = args.epochs  # Use configurable epoch count
...
if extract_data == 1:
    if data_tag == 'aecg':
        felicity.extract_felicitys_dataset_composite(overlap_pct=overlap_pct, window_size_sec=10, fs=256, data_path=data_folder, type_m_or_f=data_tag)
    else:
        # felicity.fetch_save_txt(type_m_or_f = data_tag)
        felicity.extract_felicitys_dataset(overlap_pct=overlap_pct, window_size_sec=10, fs=256, data_path=data_folder, type_m_or_f=data_tag)
...
for k in t:
    ...
    # ===================================================================
    # CRITICAL FIX: Use subject-wise split with validation
    # ===================================================================
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for epoch_counter in range(epoch):
            ...
            # ===================================================================
            # CRITICAL FIX: Normalize features before downstream tasks
            # ===================================================================
            if args.normalize_features:
                scaler_path = os.path.join(feature_saved_path, 'feature_scaler.pkl')
                x_tr_feature, x_te_feature, feature_scaler = utils.normalize_features(
...
if args.subject_wise:
    print(f"✓ Experiments used CORRECT subject-wise cross-validation")
    print(f"✓ Results represent true generalization to new subjects")
else:
    print(f"✗ WARNING: Experiments used INCORRECT within-subject split")
    print(f"✗ Results are INFLATED by 20-30% due to data leakage")
print(f"{'='*70}\n")
